...
listInList = SCH_TO_CSV_OOP_LIB.listInList
initialDirectory = "" 
	...
	if filename[-4:] == ".sch" or filename[-4:] == ".SCH":
		...
		if data_test_dump[:-1] == "EESchema Schematic File Version 2":
			...
			try:
				...
				print("Open file: {}".format(filename)) # THOM DEBUG
				
				mainFile.SetContents(f.readlines())
				
				
				for p in range (len(filename),1,-1):
					if filename[p-1] == "/":
						break
				mainFile.setSchematicName(filename[p:])
				print(filename[p:])
				#for i  in range(len(filename)-1,0,-1): # get the last part of the file path
				#	if "/" in filename[i]:
...
def sortParts():
	...
	for i in range (mainFile.get_number_of_components()):
		componentNameList.append(mainFile.getComponents()[i].getReference())
	sortList(componentNameList)
	for i in range (mainFile.get_number_of_components()):
		for p in range(i, mainFile.get_number_of_components()):
			if componentNameList[i] == mainFile.getComponents()[p].getReference():
...
def loadCSV():
	...
	if filename[-4:] == ".csv" or filename[-4:] == ".CSV":
		...
		headerItems = openCSVFile.readCSVline(data_test_dump)
		standardItems = ["SchematicName","PartType","timestamp","Reference","unit","Value","Footprint","Datasheet"]
		print(headerItems) #DEBUG
		
		if listInList(standardItems,headerItems) :
			...
			if openCSVFile.generateCSVComponents(headerItems):
